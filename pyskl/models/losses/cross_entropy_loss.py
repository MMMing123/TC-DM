...
class CrossEntropyLoss(BaseWeightedLoss):
    ...
    and ``label``.
    1) Hard label: This label is an integer array and all_file of the elements are
        in the range [0, num_classes - 1]. This label's shape should be
        ``cls_score``'s shape with the `num_classes` dimension removed.
    2) Soft label(probablity distribution over classes): This label is a
        probability distribution and all_file of the elements are in the range
    ...
    Args:
        ...
        class_weight (list[float] | None): Loss weight for each class. If set
            as None, use the same weight 1 for all_file classes. Only applies
